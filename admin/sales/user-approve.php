...
try {
  ...
  if ($sale['approval_status'] !== 'ユーザー確認待ち') {
    http_response_code(400);
    echo json_encode(['success' => false, 'message' => 'この売上は承認できません。']);
    exit;
  }
  ...
  $stmt = $pdo->prepare("
        ...
        VALUES (:tenant_id, 'ユーザー承認', 'sales_records', :row_id, :operator, :name, :details)
    ");
  $stmt->execute([
    'tenant_id' => $tenant_id,
    'row_id' => $sale_id,
  ...
  error_log('Sales user approve error: ' . $e->getMessage());
